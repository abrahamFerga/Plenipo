...
cp "$TGZ" "$CONS/cortex-ui.tgz"

cat > "$CONS/package.json" <<'EOF'
{
  "name": "cortex-ui-consumer",
  "private": true,
  "type": "module",
  "dependencies": {
...
# Reproduce the composition the README documents — the batteries-included CortexApp with a host module
...
const finance: CortexModuleUi = defineModule("finance", { tabs: { transactions: TransactionsBoard } });
const branding: CortexBranding = { name: "Acme Ops" };
...
export function App() {
  ...
  return <CortexApp moduleUi={[finance]} branding={branding} />;
}
EOF
