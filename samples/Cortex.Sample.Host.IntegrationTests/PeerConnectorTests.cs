...
/// <summary>
/// Verticals are separate systems; the cortex-peer connector is how they talk. This exercises a
...
{
    [Fact]
    public async Task Asks_a_peer_cortex_system_over_agui_and_returns_its_answer()
    {
        ...
        {
            var tools = scope.ServiceProvider.GetRequiredService<CortexPeerTools>();
            Assert.Contains("not enabled", await tools.AskPeerSystem("anything"));
        }

        (await admin.PutAsJsonAsync("/api/admin/connectors/cortex-peer/settings", new
        {
        ...
        (await admin.PostAsync("/api/admin/connectors/cortex-peer/enable", null)).EnsureSuccessStatusCode();
        ...
        {
            (await admin.PostAsync("/api/admin/connectors/cortex-peer/disable", null)).EnsureSuccessStatusCode();
        }
    }
