...
// Plenipo sample-app orchestration — the one-command way to run the FULL stack locally:
//   • Postgres (platform + audit databases) and Redis (SignalR backplane / cache)
//   • the sample API (Plenipo.Sample.Host with the Finance + Nutrition + Legal modules)
...
//   dotnet user-secrets --project samples/Plenipo.Sample.AppHost set "Parameters:ai-provider" "OpenAI"
...
//
// Run with: dotnet run --project samples/Plenipo.Sample.AppHost   (or `aspire run`)
...
var pgPassword = builder.AddParameter("plenipo-pg-password", "plenipo-dev-only", secret: true);
...
// ── API ──────────────────────────────────────────────────────────────────────
var api = builder.AddProject<Projects.Plenipo_Sample_Host>("plenipo-sample")
...
if (builder.ExecutionContext.IsRunMode && !ToolExistsOnPath("pnpm"))
{
    throw new DistributedApplicationException(
        "pnpm was not found on PATH, so the plenipo-ui / plenipo-admin-ui resources cannot start. " +
...
// services (same for the Aspire-internal "plenipo-sample-rebuilder").
