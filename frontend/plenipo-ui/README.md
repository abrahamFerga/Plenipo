...
A minimal, **server-driven** React frontend for the Plenipo .NET platform. The
...
- `VITE_API_BASE` (default `http://localhost:8080`) — base URL of the Plenipo API.
  See `.env.example`.

## Theming / branding

The domain UI's accent color is driven by CSS variables (`--plenipo-brand-50` … `--plenipo-brand-900`, as
...
```css
:root {
  --plenipo-brand-600: 220 38 38; /* primary buttons, active nav */
  --plenipo-brand-500: 239 68 68; /* hovers, focus rings */
}
```
...
export default {
  ...
  content: ["./src/**/*.{ts,tsx}", "./node_modules/@plenipo/ui/dist/**/*.js"],
};
```

```ts
// app entry — the indigo defaults; override any --plenipo-brand-* afterwards to rebrand
import "@plenipo/ui/theme.css";
```
...
`defineModule`, and pass them to `PlenipoApp` (or `AppShell`). A tab you don't
register falls back to `GenericTab` — so simple modules cost zero React.

```tsx
import { createRoot } from "react-dom/client";
import { PlenipoApp, defineModule, type ModuleTabProps } from "@plenipo/ui";
import { TransactionsBoard, BudgetPlanner } from "./finance";

// Components receive { moduleId, tab } and can use everything @plenipo/ui exports
...
```tsx
import { PermissionGate, usePermission } from "@plenipo/ui";
