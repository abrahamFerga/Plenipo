...
/**
 * The single door every request goes through: the configured transport, the configured
 * credentials, and an absolute URL off the configured base. Keeping it in one place is what lets
 * a React Native shell swap in `expo/fetch` and a bearer token without touching a call site.
 */
async function request(path: string, init: RequestInit & { headers?: Record<string, string> }): Promise<Response> {
  const { baseUrl, authHeaders, fetch: doFetch } = clientConfig();
...
/**
 * Thin fetch wrapper that prefixes the API base URL and attaches the configured auth headers.
...
/** The product's public identity from GET /api/platform/branding. */
export interface Branding {
  name: string;
}

/** Which push service a device's token belongs to. Mirrors C# `DevicePlatform`. */
export type DevicePlatformName = "Ios" | "Android" | "Web";

/** What a client sends to register itself for push notifications. */
export interface DeviceRegistration {
  /** A stable id for this installation, minted by the client and kept across token rotations. */
  installationId: string;
  /** The push service token to deliver to. Goes one way — no endpoint ever returns it. */
  pushToken: string;
  platform: DevicePlatformName;
  /** Optional human-readable label ("Pixel 9") for a "your devices" list. */
  deviceName?: string;
}

/** One of the caller's registered devices, as the server describes it — deliberately token-less. */
...
/**
 * A file picked on a platform with no DOM `File` — React Native's camera roll, camera, and
 * document pickers all hand back a URI, and its `FormData` takes this descriptor as the part
 * value directly.
 */
...
/** Anything the platform file store can take: a web `File`/`Blob`, or a native URI descriptor. */
export type UploadableFile = File | NativeFileRef;
 ...
 */
export async function uploadFile(file: UploadableFile): Promise<StoredFileInfo> {
  const form = new FormData();
  if ("uri" in file) {
    // React Native's FormData reads the file name and content type off the descriptor itself —
    // there is no Blob to hand it, and no third filename argument.
    form.append("file", file as unknown as Blob);
  } else {
    form.append("file", file, file.name);
  }

  // No Content-Type header on purpose: the transport sets it, with the multipart boundary.
...
export const api = {
  ...
  // The product's own name, resolved at runtime from host configuration. Anonymous by design:
  // it must render before any sign-in, which is what lets one prebuilt shell serve every product.
  branding: () => apiGet<Branding>("/api/platform/branding"),
  ...
  files: {
    ...
    downloadUrl: (id: string) => `${apiBase()}/api/files/${id}`,
  },

  // The current user's in-app notification inbox (strictly self-scoped server-side).
  notifications: {
    // Push devices. `register` is idempotent per installation, so a shell calls it on every
    // launch — push tokens rotate, and matching on installationId turns that into an update
    // rather than a pile of dead registrations. `list` never returns the tokens themselves.
    devices: {
      list: () => apiGet<RegisteredDevice[]>("/api/notifications/devices"),
      register: (device: DeviceRegistration) => apiSend("/api/notifications/devices", "PUT", device),
      forget: (installationId: string) =>
        apiSend(`/api/notifications/devices/${encodeURIComponent(installationId)}`, "DELETE"),
    },
