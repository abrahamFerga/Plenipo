...
import { createStreamingDecoder } from "./utf8";
...
export async function* runAgui(
  ...
  const res = await doFetch(`${baseUrl}/api/agui/${moduleId}`, {
    method: "POST",
    headers: {
  ...
  // A body-less response here is usually a transport that cannot stream rather than a server
  // fault — React Native's built-in fetch buffers instead of exposing `body`. Say so, because
  // "undefined is not an object" three frames down would not.
  if (!res.ok) {
  ...
  if (!res.body) {
    throw new ApiError(
      res.status,
      res.statusText,
      "AG-UI run failed: this fetch implementation does not expose a streaming response body. " +
        "Configure the client with a streaming fetch (React Native: `fetch` from `expo/fetch`).",
    );
  }
