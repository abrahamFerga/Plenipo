...
Thanks for your interest! Cortex is a **base platform for AI-first, chat-first apps**; most contributions
...
```
Cortex.slnx                  # the base platform — publishable packages + the bare host
...
samples/Cortex.Samples.slnx  # example apps built ON the platform
  Cortex.Modules.{Finance,Nutrition,Legal}  # demo verticals (+ .Tests)
...
local dev; in production they'd consume the `Cortex.*` NuGet packages.

## Build & test

```bash
# Platform
dotnet build Cortex.slnx
dotnet test  Cortex.slnx
...
- **Tests** — add unit tests for new logic; for endpoint behaviour, prefer an integration test in
  `Cortex.Sample.Host.IntegrationTests` (it exercises the full auth + RBAC pipeline).

## Adding a module

A new vertical is a class implementing `IModule`. For a step-by-step walkthrough see
[BUILDING_A_MODULE.md](BUILDING_A_MODULE.md) (worked example: `samples/Cortex.Modules.Tasks`); for fuller
references see `samples/Cortex.Modules.Legal` (smallest stateless) or `Cortex.Modules.Finance` (owns
persistence). In short:

1. Reference `Cortex.Modules.Sdk` (+ `Application`, `Core`).
...
4. Install it in a host: `builder.AddCortexModule<YourModule>()`.
