...
public static class ConnectorRegistration
{
    ...
    /// </summary>
    public static IHostApplicationBuilder AddPlenipoConnectorsFrom(
        this IHostApplicationBuilder builder, params Assembly[] assemblies)
    {
        foreach (var assembly in assemblies)
        {
            ...
            {
                builder.AddPlenipoConnectorInstance((IConnector)Activator.CreateInstance(type)!);
            }
        }
    ...
    /// both the scan above and the per-type <c>AddPlenipoConnector&lt;T&gt;()</c> share, and the
    /// entry point for connectors without a parameterless constructor.
    /// </summary>
    public static IHostApplicationBuilder AddPlenipoConnectorInstance(
