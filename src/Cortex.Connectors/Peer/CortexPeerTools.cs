    ...
    OutboundUrlPolicy outboundUrls)
{
    ...
    [Description("Ask the connected peer Cortex system's agent a question (e.g. ask the finance system about a client's invoices from here). Returns the peer agent's answer. The peer enforces its own permissions and audits the exchange.")]
    ...
    {
        var values = await settings.GetAsync(CortexPeerConnector.ConnectorId, cancellationToken);
        if (values is null ||
            !values.TryGetValue(CortexPeerConnector.BaseUrlSetting, out var baseUrl) ||
            !values.TryGetValue(CortexPeerConnector.ModuleIdSetting, out var moduleId) ||
            string.IsNullOrWhiteSpace(baseUrl) || string.IsNullOrWhiteSpace(moduleId))
        {
            return NotConfigured;
        }

        var peerName = values.TryGetValue(CortexPeerConnector.PeerNameSetting, out var name) && !string.IsNullOrWhiteSpace(name)
            ? name
            : "the peer system";
        ...
        if (values.TryGetValue(CortexPeerConnector.AuthHeaderNameSetting, out var headerName) &&
            values.TryGetValue(CortexPeerConnector.AuthHeaderValueSetting, out var headerValue) &&
        ...
        try
        {
            var client = httpClients.CreateClient(CortexPeerConnector.HttpClientName);
...
public sealed class CortexPeerToolSource : IConnectorToolSource
{
    public string ConnectorId => CortexPeerConnector.ConnectorId;

    public IReadOnlyList<ModuleTool> GetTools(IServiceProvider scopedServices)
    {
        var tools = scopedServices.GetRequiredService<CortexPeerTools>();
        return
        [
            new ModuleTool
            {
