    ...
    private async Task ExecuteJobAsync(Guid jobId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (job.UserId == BackgroundJob.SystemUserId)
            {
                ...
                context.SetIdentity("system:recurring", "Plenipo scheduler");
            }
            else
            {
            ...
            && job.Status is JobStatus.Succeeded or JobStatus.Failed)
        {
            try
            {
                var notifier = services.GetRequiredService<Plenipo.Application.Notifications.INotifier>();
                await notifier.NotifyAsync(new Plenipo.Application.Notifications.Notification(
                    job.TenantId,
                    job.UserId,
                    Category: "jobs",
