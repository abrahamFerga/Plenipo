...
public static class AdminEndpoints
{
    ...
    private static void MapNotificationSettings(RouteGroupBuilder group)
    {
        ...
        group.MapPut("/notification-settings", async (
            ...
            Plenipo.Application.Secrets.ISecretVault vault, ICurrentUser current,
    ...
    /// <summary>Must match WebhookNotificationChannel.SecretScope (Plenipo.Infrastructure).</summary>
    private const string WebhookSecretScope = "Plenipo.Notifications.WebhookSecret";
    ...
    private static void MapTenants(RouteGroupBuilder group)
    {
        ...
        group.MapPost("/tenants/provision", async (
            [FromBody] ProvisionTenantRequest body, Plenipo.Application.Commerce.ITenantProvisioningService provisioning,
            IModuleCatalog catalog, CancellationToken ct) =>
        {
            var result = await provisioning.ProvisionAsync(new Plenipo.Application.Commerce.ProvisionTenantCommand(
            ...
            {
                Plenipo.Application.Commerce.ProvisionError.Invalid => Results.BadRequest(result.ErrorDetail),
                Plenipo.Application.Commerce.ProvisionError.SlugTaken => Results.Conflict(result.ErrorDetail),
    ...
    private static void MapSecurity(RouteGroupBuilder group)
    {
        ...
        group.MapGet("/security/catalog", (
            IModuleCatalog catalog, Plenipo.Application.Connectors.IConnectorCatalog connectors) =>
        {
