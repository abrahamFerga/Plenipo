...
/// <summary>
/// Serves the product's <b>domain UI</b> — the <c>@plenipo/ui</c> SPA, built with the product's own
...
public static class DomainUiExtensions
{
    ...
    /// </summary>
    public static WebApplication UsePlenipoDomainUi(this WebApplication app, string? assetPath = null)
    {
        var logger = app.Services.GetRequiredService<ILoggerFactory>().CreateLogger("Plenipo.DomainUi");
        ...
        if (!Directory.Exists(root))
        {
            if (logger.IsEnabled(LogLevel.Information))
            {
                logger.LogInformation(
                    "Plenipo domain UI assets not found at {Path}; the SPA will not be served from this host. " +
                    "Build @plenipo/ui with your product's branding and copy its dist/ output there to enable it.",
                    root);
            }
            return app;
        }
        ...
        if (logger.IsEnabled(LogLevel.Information))
        {
            logger.LogInformation("Plenipo domain UI mounted at / from {Path}", root);
        }
        return app;
    }
}
