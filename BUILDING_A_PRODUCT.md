...
| 8 | Shape identity and roles | `Auth:` config — `PermissionSource` (`Database`/`Token`), `DefaultRole` (what a JIT user gets when their token asserts nothing; empty = permission-less). Declare PRODUCT roles / reshape built-in baselines with `AddPlenipoRole("paralegal", [...])`. The declaration is **live, not a seed**: add a permission (or a whole role) in a later release and it reaches every existing tenant on restart, with no migration or per-tenant repair — while a permission a tenant admin removed in Admin → Roles stays removed. `AddPlenipoRole(..., replace: true)` narrowing propagates the same way. `system_admin` is never customizable | `samples/Plenipo.Sample.Host/Program.cs` |
