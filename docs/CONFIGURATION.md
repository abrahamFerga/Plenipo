...
| `Auth:Dev` | `RolesWhenAbsent` | Development only. The roles an ABSENT `X-Dev-Roles` header asserts. Unset = `system_admin` (the convenience that makes a fresh clone operable with no headers); set it to `""` so absence means no roles — an absent header is what a header-stripping proxy produces, and a proxy must degrade a caller, never escalate one. A present-but-empty header always means no roles |
