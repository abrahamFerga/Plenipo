...
public sealed class ConnectorMarketplaceTests
{
    private static readonly string[] BundledIds =
        ["local-folder", "azure-blob", "s3", "msgraph", "google-drive", "documenso", "cortex-peer"];
    ...
    public async Task Admin_catalog_splits_installed_from_available_when_config_excludes_one()
    {
        ...
        Assert.Equal("Cortex.Connectors", documenso.GetProperty("package").GetString());
    ...
    private sealed class ExcludeDocumensoFactory : CortexApiFactory
    {
