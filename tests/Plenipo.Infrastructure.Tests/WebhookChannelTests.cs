...
public sealed class WebhookChannelTests
{
    ...
    public async Task Send_PostsSignedPayload_ToTheConfiguredUrl()
    {
        ...
        var channel = new WebhookNotificationChannel(
            new FixedConfig(new NotificationWebhookConfig("https://hooks.example/plenipo", secretRef)),
            vault,
        ...
        Assert.Equal("https://hooks.example/plenipo", handler.Request!.RequestUri!.ToString());
