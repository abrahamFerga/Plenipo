...
/// </summary>
public sealed class UserInviteTests : IClassFixture<PlenipoApiFactory>
{
    private readonly PlenipoApiFactory _factory;

    public UserInviteTests(PlenipoApiFactory factory) => _factory = factory;
    ...
    public async Task Duplicates_and_existing_members_are_rejected_with_a_reason()
    {
        ...
        var self = await admin.PostAsJsonAsync("/api/admin/users/invites",
            new { email = "dev@plenipo.local", roles = Array.Empty<string>() });
